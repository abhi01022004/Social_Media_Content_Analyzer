    ...
    <div className="card" id="analysis-card" style={{ marginTop: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <div className="small" style={{ marginTop: 4 }}>
            Automatic extraction, NLP description & suggestions
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 8 }}>
        <div className="card" style={{ padding: 10 }}>
          <strong>Type</strong>
          <div className="small" style={{ marginTop: 6 }}>{data.description?.type || 'Unknown'}</div>
        </div>

        <div className="card" style={{ padding: 10 }}>
          <strong>Confidence</strong>
          <div className="small" style={{ marginTop: 6 }}>
            {data.description?.confidence ? (data.description.confidence * 100).toFixed(0) + '%' : '—'}
          </div>
        </div>
      ...
      <div className="suggestions">
        <ul>
          {data.suggestions && data.suggestions.length ? data.suggestions.map((s, i) => <li key={i}>{s}</li>) : <li>No suggestions</li>}
        </ul>
      </div>
    </div>
  );
